...
Все тесты используют tiny watson модель — без скачивания весов.
"""
...
# ---------------------------------------------------------------------------
# Fixture: минимальная watson-модель вместо BERT
# ---------------------------------------------------------------------------
...
@pytest.fixture
def nlp_module(watson_model, optimizer_cfg):
...
@pytest.fixture
def nlp_module_with_weights(watson_model, optimizer_cfg):
    from src.training.module import NLPModel
...
class TestLossCalculation:
    ...
    def test_no_class_weights_uses_model_loss(self, nlp_module):
        ...
        outputs = nlp_module(**batch)
        loss = nlp_module._calculate_loss(outputs, batch["labels"])
        # outputs.loss вычислен watsonEncoder с nn.CrossEntropyLoss без весов
...
class TestConfigureOptimizers:
    ...
    def test_returns_dict_with_scheduler(self, watson_model):
        from src.training.module import NLPModel
        ...
        with patch("src.training.module.instantiate") as mock_instantiate:
            optimizer = torch.optim.AdamW(watson_model.parameters(), lr=1e-3)
    ...
    def test_only_trainable_params_passed_to_optimizer(self, watson_model):
        """Замороженные параметры не должны попасть в оптимизатор."""
        from src.training.module import NLPModel

        # Замораживаем все параметры модели
        for param in watson_model.parameters():
...
class TestThresholdSearch:
    def test_target_precision_logs_threshold(self, watson_model):
        from src.training.module import NLPModel
