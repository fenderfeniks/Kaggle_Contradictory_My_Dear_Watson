...
IMAGE = Variable.get("PROJECT_IMAGE", default_var="my-company/kaggle_my_dear_watson:api-latest")
...
with DAG(
    ...
    promote_model = KubernetesPodOperator(
        ...
        env_from=[
            k8s.V1EnvFromSource(
                config_map_ref=k8s.V1ConfigMapEnvSource(name="my-dear-watson-api-config")
            ),
        ],
        get_logs=True,
    ...
    restart_api = KubernetesPodOperator(
        ...
        cmds=["kubectl", "rollout", "restart", "deployment/my-dear-watson-api", "-n", NAMESPACE],
        get_logs=True,
        is_delete_operator_pod=True,
        service_account_name="airflow-worker-sa",
    )
